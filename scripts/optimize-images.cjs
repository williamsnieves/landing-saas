...
console.log(
  '📊 Original size:',
  (fs.statSync(heroImage).size / 1024).toFixed(2),
  'KB'
);
...
console.log(
  '   2. Use ImageMagick: convert hero-dashboard.png -quality 80 hero-dashboard.webp'
);
console.log(
  '   3. Use sharp package: npm install sharp && node optimize-with-sharp.js'
);
console.log(
...
try {
  ...
  sharp(heroImage)
    ...
    .then((info) => {
      ...
      console.log(
        '💾 Saved:',
        ((1 - info.size / fs.statSync(heroImage).size) * 100).toFixed(1),
        '% reduction'
      );
